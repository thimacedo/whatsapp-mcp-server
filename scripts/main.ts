...
async function start() {
  ...
  const hasCreds = await Bun.file(`${AUTH_DIR}/creds.json`).exists().catch(() => false);
  if (!hasCreds && await Bun.file(AUTH_DIR).exists().catch(() => false)) {
    console.log('Limpando credenciais antigas...');
    await Bun.rm(AUTH_DIR, { recursive: true });
  }
